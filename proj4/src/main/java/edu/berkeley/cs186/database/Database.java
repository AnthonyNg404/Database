...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                LockContext rootContext = lockManager.databaseContext();
                List<Lock> locks = lockManager.getLocks(getTransaction());
                Set<LockContext> seen = new HashSet<>();
                if (locks != null && !locks.isEmpty()) {
                    for (Lock lock : locks) {
                        seen.add(LockContext.fromResourceName(lockManager, lock.name));
                    }
                    Deque<LockContext> lockContextDQ = new ArrayDeque<>();
                    Deque<LockContext> lockContextDQ1 = new ArrayDeque<>();
                    lockContextDQ.addLast(rootContext);

                    while (!lockContextDQ.isEmpty()) {
                        LockContext curContext = lockContextDQ.pollFirst();
                        if (seen.contains(curContext)) {
                            lockContextDQ1.addLast(curContext);
                        }
                        Map<String, LockContext> children = curContext.getChildrenContext();
                        if (children != null && !children.isEmpty()) {
                            for (LockContext childContext : children.values()) {
                                    lockContextDQ.addLast(childContext);
                            }
                        }
                    }
//                    System.out.println(lockContextDQ1);
//                    System.out.println(locks);

                    while (!lockContextDQ1.isEmpty()) {
                        LockContext curContext = lockContextDQ1.pollLast();
//                        System.out.println(curContext);
                        curContext.release(getTransaction());
                    }
                }
                return;
            } catch (Exception e) {
