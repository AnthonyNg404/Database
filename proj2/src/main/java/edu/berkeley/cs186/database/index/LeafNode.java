...
class LeafNode extends BPlusNode {
        ...
        if (keys.contains(key)) {
            ...
            if (keys.size() == 2 * metadata.getOrder()) {
                ...
                for (int i = 0; i < metadata.getOrder() + 1; i++) {
                    newKeys.add(keys.get(metadata.getOrder()));
                    keys.remove(metadata.getOrder());
                    newRids.add(rids.get(metadata.getOrder()));
                ...
                sync();
                newNode.sync();
                return Optional.of(new Pair<DataBox, Long>(newKeys.get(0), newNode.page.getPageNum()));
            } else {
                int index = InnerNode.numLessThan(key, keys);
                keys.add(index, key);
                rids.add(index, rid);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        while ((float) keys.size() / (metadata.getOrder() * 2) < fillFactor && data.hasNext()) {
            Pair<DataBox, RecordId> dataPoint = data.next();
            if (keys.contains(dataPoint.getFirst())) {
                throw new BPlusTreeException("Duplicate keys!");
            } else {
                put(dataPoint.getFirst(), dataPoint.getSecond());
            }
        }
        if (data.hasNext()) {
            List<DataBox> newKeys = new ArrayList<>();
            ...
            LeafNode newNode = new LeafNode(metadata, bufferManager, newKeys, newRids, rightSibling, treeContext);
            Pair<DataBox, RecordId> dataPoint = data.next();
            newNode.put(dataPoint.getFirst(), dataPoint.getSecond());
            rightSibling = Optional.of(newNode.page.getPageNum());
            sync();
            newNode.sync();
            return Optional.of(new Pair<DataBox, Long>(dataPoint.getFirst(), newNode.page.getPageNum()));
        }
        return Optional.empty();
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long rightSiblingIndex = buf.getLong();
        Optional<Long> rightSibling;
        if (rightSiblingIndex == -1) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(rightSiblingIndex);
        }
