...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (root != null) {
            ...
            if (ret.isPresent()) {
                ...
                newKeys.add(ret.get().getFirst());
                newChildren.add(root.getPage().getPageNum());
                newChildren.add(ret.get().getSecond());
                this.updateRoot(new InnerNode(metadata, bufferManager, newKeys, newChildren, lockContext));
            }
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (index == leaf.getKeys().size() && !leaf.getRightSibling().isPresent()) {
                return false;
            } else if (index == leaf.getKeys().size() && leaf.getRightSibling().isPresent()) {
        ...
        public RecordId next() {
            if(hasNext()) {
                RecordId nextID = leaf.getRids().get(index);
                index++;
                return nextID;
            } else {
