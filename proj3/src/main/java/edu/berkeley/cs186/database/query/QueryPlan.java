...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        if (cols.size() > 0) {
            for (int idx : cols) {
                QueryOperator newMinOp = new IndexScanOperator(this.transaction, table, selectPredicates.get(idx).column, selectPredicates.get(idx).operator, selectPredicates.get(idx).value);
                int newMinCost = newMinOp.estimateIOCost();
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> pre : prevMap.keySet()) {
            for (JoinPredicate join : joinPredicates) {
                ...
                if (pre.contains(leftTable) && !pre.contains(rightTable)) {
                    Set<String> newKey = new HashSet<>();
                    newKey.add(rightTable);
                    QueryOperator leftOp = prevMap.get(pre);
                    QueryOperator rightOp = pass1Map.get(newKey);
                    QueryOperator joinOp = minCostJoinType(leftOp, rightOp, leftColumn, rightColumn);

                    int cost = joinOp.estimateIOCost();
                    Set<String> joinKey = new HashSet<>(pre);
                    joinKey.add(rightTable);
                    if (!result.containsKey(joinKey)) {
                        result.put(joinKey, joinOp);
                    } else if (result.get(joinKey).estimateIOCost() > cost) {
                        result.put(joinKey, joinOp);
                    }
                } else if (pre.contains(rightTable) && !pre.contains(leftTable)) {
                    Set<String> newKey = new HashSet<>();
                    newKey.add(leftTable);
                    QueryOperator leftOp = pass1Map.get(newKey);
                    QueryOperator rightOp = prevMap.get(pre);

                    QueryOperator joinOp = minCostJoinType(rightOp, leftOp, rightColumn, leftColumn);

                    int cost = joinOp.estimateIOCost();
                    Set<String> joinKey = new HashSet<>(pre);
                    joinKey.add(leftTable);
                    if (!result.containsKey(joinKey)) {
                        result.put(joinKey, joinOp);
                    } else if (result.get(joinKey).estimateIOCost() > cost) {
                        result.put(joinKey, joinOp);
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> curPass = new HashMap<>();
        ...
        Map<Set<String>, QueryOperator> prevPass = new HashMap<>(curPass);

        while (prevPass.size() > 1) {
            prevPass = minCostJoins(prevPass, curPass);
        }

        Set<String> finalKey = new HashSet<>(tableNames);
        finalOperator = prevPass.get(finalKey);
