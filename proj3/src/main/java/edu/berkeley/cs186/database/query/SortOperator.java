...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        ArrayList<Record> recordsCopy = new ArrayList<>();
        while (records.hasNext()) {
            recordsCopy.add(records.next());
        }
        recordsCopy.sort(comparator);
        return makeRun(recordsCopy);
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        ArrayList<Iterator<Record>> iterators = new ArrayList<Iterator<Record>>();
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        ArrayList<Run> mergedPass = new ArrayList<>();
        for (int i = 0; i < (runs.size() / numBuffers - 1) + 2; i++) {
            mergedPass.add(mergeSortedRuns(runs.subList(i * (numBuffers-1), (i+1) * (numBuffers-1))));
        }

        if (mergedPass.size() > 0) {
            return mergedPass;
        }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            BacktrackingIterator<Record> run = QueryOperator.getBlockIterator(sourceIterator, getSource().getSchema(), numBuffers);
